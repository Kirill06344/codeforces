...
func main() {
	...
	for tc := 0; tc < t; tc++ {
		...
		var zeroModSubstrings long = 0
		for i := 0; i < 3; i++ {
			zeroModSubstrings += cnt[i] * (cnt[i] - 1) / 2
		}
		validSum := total - zeroModSubstrings

		var subtractAlt long = 0
		var length long = 1
		...
		writeLine(validSum - subtractAlt)
	}
}
